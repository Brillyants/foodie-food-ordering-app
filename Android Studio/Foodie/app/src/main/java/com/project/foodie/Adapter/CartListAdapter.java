...
public class CartListAdapter extends RecyclerView.Adapter<CartListAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.totalEachItem.setText(String.valueOf(Math.round((foodDomains.get(position).getNumInCard() * foodDomains.get(position).getPrice()) * 100.0) / 100.0));
        holder.num.setText(String.valueOf(foodDomains.get(position).getNumInCard()));
        Picasso.with(holder.itemView.getContext()).load(foodDomains.get(position).getPic()).into(holder.pic);
